...
<body class="min-h-screen">
        ...
        <section id="form-pemesanan" class="py-12 md:py-16">
            ...
            <div class="max-w-3xl mx-auto p-6 bg-purple-100 rounded-xl shadow-xl coquette-border">
                <form id="order-form" class="space-y-6">
                    ...
                    <!-- Tombol Konfirmasi yang akan terhubung ke WhatsApp -->
                    <button type="submit" class="coquette-button w-full py-4 rounded-full font-extrabold text-xl hover:shadow-xl">
                        Konfirmasi Pesanan Selesai & Lanjut ke WhatsApp
                    </button>
                    <div id="order-message" class="text-center mt-3 text-lg font-bold text-green-600 hidden p-3 bg-green-50 rounded-lg">Pesananmu telah dikonfirmasi dan detailnya sudah disiapkan di WhatsApp. Silakan kirim pesan tersebut!</div>
                </form>
            </div>
        </section>
    ...
    <script type="module">
        ...
        // WhatsApp number in international format (62 is Indonesia's code)
        const WHATSAPP_NUMBER = '6281211234744'; 
        ...
        // --- CART & TESTIMONIALS LOGIC (Firestore Integration) ---
        ...
        function loadCartFromFirestore() {
            const cartRef = getCartDocRef();
            if (cartRef) {
                // Attach listener only after auth is ready and userId is set
                if (!userId) return;

                onSnapshot(cartRef, (docSnap) => {
                    if (docSnap.exists() && docSnap.data().items) {
                        // Deserialize the cart array
                        cart = docSnap.data().items.map(item => ({
                             ...item,
        ...
        // Loads and renders testimonials in real-time
        function loadTestimonials() {
            const testiRef = getTestimonialsCollectionRef();
            if (testiRef) {
                // Query: Ambil semua testimoni. Gunakan query() tanpa orderBy.
        ...
        // Fungsi Helper: Format Rupiah (digunakan di Cart UI dan Order Form)
        ...
        function updateCartUI() {
            ...
            // 2. Update Cart Modal (The requested feature is implemented here)
            ...
            if (cart.length === 0) {
                ...
                cart.forEach(item => {
                    // Ensure the price is displayed correctly for single item in modal
                    const pricePerItem = formatRupiah(item.price); 
                    const itemElement = document.createElement('div');
                    itemElement.className = 'flex justify-between items-center text-gray-700 border-b pb-2';
                    itemElement.innerHTML = `
                        <div>
                            <span class="font-semibold">${item.name}</span>
                            <span class="text-sm text-gray-500"> (${pricePerItem} / pcs)</span>
                        </div>
                        <div class="flex items-center space-x-3">
        ...
        window.addToCart = function(product) {
            ...
            if (existingItem) {
                ...
                cart.push({ ...product, quantity: 1, price: Number(product.price) }); // Ensure price is stored as number
            }
        ...
        // Event Listener untuk Formulir Pemesanan (DENGAN INTEGRASI WHATSAPP)
        document.getElementById('order-form').addEventListener('submit', async function(e) {
            ...
            submitBtn.disabled = true;
            submitBtn.textContent = 'Memproses Pesanan & Menyiapkan WA...';

            const ordersCollectionRef = getOrdersCollectionRef();
            const total = cart.reduce((sum, item) => sum + item.price * item.quantity, 0);
            const totalRupiah = formatRupiah(total);

            // Buat daftar item untuk pesan WhatsApp
...
Halo Pinky Bracelet! Saya ingin mengonfirmasi pesanan saya.
            ...
            if (ordersCollectionRef) {
                try {
                    const orderData = {
                        ...
                        userId: userId,
                        status: 'Pending Payment - WA Contacted'
                    };
                    ...
                    // 3. Hapus keranjang setelah pesanan berhasil
                    cart = [];
                    await updateCartInFirestore(); // Ini akan mengosongkan keranjang di Firestore dan memicu updateCartUI
                    
                    // 4. Tampilkan pesan sukses dan reset form
                    orderMessage.classList.remove('hidden');
                    document.getElementById('order-form').reset();
                    
                    showNotification("Pesanan berhasil dikonfirmasi! Silakan kirim pesan di WhatsApp.");
                    ...
                    submitBtn.disabled = false;
                    submitBtn.textContent = 'Konfirmasi Pesanan Selesai & Lanjut ke WhatsApp';
